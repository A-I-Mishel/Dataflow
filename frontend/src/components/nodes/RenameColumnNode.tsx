...
export default function RenameColumnNode({ id, data, selected }: NodeProps) {
  ...
  const getFreshMapping = (): Record<string, string> =>
    usePipelineStore.getState().nodes.find((node) => node.id === id)?.data.config.mapping ?? {};
  ...
  const handleRemoveMapping = (oldKey: string): void => {
    ...
    // The removed row unmounts and re-keys siblings (see key comment
    // below) — land focus on Add instead of dropping it to <body>.
    requestAnimationFrame(() => addButtonRef.current?.focus());
  };

  const handleOldNameChange = (event: ChangeEvent<HTMLInputElement>, oldKey: string): void => {
  ...
  const handleNewNameChange = (event: ChangeEvent<HTMLInputElement>, key: string): void => {
    updateNodeConfig(id, {
  ...
  return (
    ...
    >
      {entries.length === 0 && <p className={hintClass}>No mappings defined</p>}
      ...
      <button ref={addButtonRef} type="button" onClick={handleAddMapping} className={addBtnClass}>
        Add Mapping
      </button>
    </NodeShell>
  );
