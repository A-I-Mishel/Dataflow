...
// Category hues come from the cat-* design tokens (single source of truth),
// not raw palette classes — identical rendering in both themes either way,
// but token changes now propagate to every node at once.
const TONE_STYLES: Record<NodeTone, { dot: string; iconBox: string; icon: string }> = {
  blue: {
    dot: "bg-cat-cleaning",
    iconBox: "bg-elevated border border-line",
    icon: "text-cat-cleaning",
  },
  purple: {
    dot: "bg-cat-transform",
    iconBox: "bg-elevated border border-line",
    icon: "text-cat-transform",
  },
  orange: {
    dot: "bg-cat-encode",
    iconBox: "bg-elevated border border-line",
    icon: "text-cat-encode",
  },
};
...
export const removeBtnClass = "text-xs font-medium text-ink3 transition-colors hover:text-danger";
...
export default function NodeShell({
  ...
  return (
    <div
      ...
      } ${hasError || runFailed ? "border-danger ring-2 ring-danger" : ""}`}
    >
        ...
        <div className="node-drag-handle relative flex cursor-grab select-none items-center gap-2.5 border-b border-linesoft px-3.5 py-2.5 active:cursor-grabbing">
          ...
          {runStatus === "done" && (
            <span
              ...
              className="ml-auto h-2.5 w-2.5 shrink-0 rounded-full bg-ok ring-4 ring-ok/20"
            />
          )}
          {runStatus === "error" && (
            <span
              title="Run failed"
              className="ml-auto h-2.5 w-2.5 shrink-0 rounded-full bg-danger ring-4 ring-danger/20"
            />
          )}
          {runStatus === undefined && (
            <span
              title={hasError ? "Needs attention" : configured ? "Configured" : "Default"}
              className={`ml-auto h-2.5 w-2.5 shrink-0 rounded-full ring-4 ${hasError ? "bg-danger ring-danger/20" : configured ? "bg-ok ring-ok/20" : "bg-muted ring-ink3/20"}`}
            />
          )}
        </div>
        {hasError && (
          <div className="space-y-1 border-b border-danger/20 bg-danger/10 px-4 py-2">
            ...
            {errors.length > 3 && (
              <p className="pl-3.5 text-[11px] font-semibold text-danger/80">
