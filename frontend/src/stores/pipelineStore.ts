...
export interface PipelineState {
  ...
  isLargeFile: boolean;
  /** Bumped on template/saved-pipeline loads so the canvas reframes. Transient — never persisted. */
  fitSignal: number;
...
/** Minimal shape the persist layer reads — exported for unit tests. */
...
export function migratePersistedState(persisted: unknown): unknown {
  // v0/v1 stored the full upload preview (rows) — strip it so old
  // payloads shrink to metadata on next save instead of blowing the
  // ~5MB localStorage quota.
  if (typeof persisted !== "object" || persisted === null) return persisted;
  const state = persisted as Record<string, unknown>;
  const original = state.originalData as DataPreview | null | undefined;
  if (original !== null && original !== undefined && typeof original === "object") {
    state.originalData = { ...original, preview: [] };
  }
  return persisted;
}

export function partializePipelineState(state: PipelineState) {
  // Persist dataset METADATA only — never preview rows. Preview rows
  // are what blows the localStorage quota on wide CSVs, and they go
  // stale the moment the backend restarts anyway. Column lists, dtypes
  // and cardinality stats stay so dropdowns/forecasts keep working;
  // checkSession() clears the dead sessionId on rehydrate.
    ...
    originalData:
      original === null
        ...
        : {
            ...
            dtypes: original.dtypes,
            row_count: original.row_count,
            missing_values: original.missing_values,
            large: original.large,
            filename: original.filename,
            unique_counts: original.unique_counts,
            cardinality_available: original.cardinality_available,
            one_hot_max_cells: original.one_hot_max_cells,
            preview: [],
          },
...
export const usePipelineStore = create<PipelineState>()(
  persist(
    (set, get) => ({
      ...
      checkSession: () => {
        const state = get();
        // Backend sessions are in-memory: after a reload the sessionId is
        // almost certainly dead, and a restored preview would be stale anyway
        // (we persist metadata only, never preview rows). Clear the session so
        // the Run gate asks for a re-upload, but keep columnList/metadata so
        // dropdowns and cardinality forecasts still work.
        if (
          state.sessionId &&
          (state.originalData === null || state.originalData.preview.length === 0)
        ) {
          set({ sessionId: null });
        }
      },
    ...
    }),
    {
      ...
      partialize: partializePipelineState,
    },
  ),
);
