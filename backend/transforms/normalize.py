"""FILE: backend/transforms/normalize.py
PURPOSE: 'normalize' step — one pandas op + pandas code string.
HOW IT FITS: engine.py _TRANSFORMS["normalize"] -> apply_normalize(df, config) -> (new_df, code_line).
CONFIG: config.columns, config.method. Pandas: df.select_dtypes(...). Missing column -> 400 error.
